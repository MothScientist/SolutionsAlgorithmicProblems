...
# что левее некоторой точки в нем после этой операции лежат элементы, удовлетворяющие заданному предикату,
...
# то есть итератор (индекс) на конец части, которая содержащих элементы, удовлетворяющие заданному предикату.
#
# В таком виде вам будет удобно использовать эту функцию для реализации сортировки.

def partition(_lst, pivot):
    less_pivot = [i for i in _lst if i < pivot]
    more_pivot = [i for i in _lst if i >= pivot]

    return less_pivot, more_pivot

_n = input()
input_lst = list(map(int, input().split()))
input_pivot = int(input())

a, b = partition(input_lst, input_pivot)
