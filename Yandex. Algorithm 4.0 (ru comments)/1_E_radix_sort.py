...
# Для каждой цифры создается «корзина» («bucket»),
...
# Таким образом, после первой фазы строки отсортированы по последней цифре,
# после двух фаз — по двум последним, ..., после m фаз — по всем.
# При важно, чтобы элементы в корзинах сохраняли тот же порядок, что и в исходном массиве (до начала этой фазы).
# Например, если массив до первой фазы имеет вид: 111, 112, 211, 311,
# то элементы по корзинам распределятся следующим образом: в первой корзине будет.
# 111, 211, 311, а второй: 112. Напишите программу, детально показывающую работу этого алгоритма на заданном массиве.
#
# Формат ввода:
...
def radix_sort(arr):
    ...
    with open("output.txt", "w") as output_file:
        print("Initial array:", file=output_file)
        print(*arr, sep=", ", file=output_file)
        print("**********", file=output_file)
        for phase in range(max_len-1, -1, -1):
            buckets = [[] for _ in range(10)]
            for string in arr:
                if len(string) > phase:
                    buckets[int(string[phase])].append(string)
                else:
                    buckets[0].append(string)
            print("Phase", max_len - phase, file=output_file)
            for i, bucket in enumerate(buckets):
                print(f"Bucket {i}:", ", ".join(bucket) if bucket else "empty", file=output_file)
            arr = [string for bucket in buckets for string in bucket]
            print("**********", file=output_file)
        print("Sorted array:", file=output_file)
        print(*arr, sep=", ", file=output_file)


with open("input.txt", "r") as input_file:
    n = int(input_file.readline())
    array = [line.strip() for line in input_file]
